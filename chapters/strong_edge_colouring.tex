...
Brualdi and Quinn Massey conjectured made the following conjecture
in 1993:

\begin{conjecture}[Brualdi, Quinn Massey \cite{brualdiIncidenceStrongEdge1993}]
...
$\Delta(G)$ red vertices, $p\Delta(B)$ black vertices and the components of
black/blue and red/green vertices form a bipartition $G=A\sqcup B$ where
$A$, $B$ are $\Delta(G)$-regular and $p\Delta(A)$-regular respectively.
...
which is $p\Delta(A)$ regular. Hence $\phi(\ext_i^\sigma)=p$. However this
...
$|E(H[N_s(e)])| \leq \lambda(p) \Delta(G)^4$.
...
We can calculate that WLOG $\Delta(H)=2p\Delta(G)^2$ so $\sigma=1-\lambda(p)/2p^2$
...
$p$. This suggests that the bound $\lambda(p)$ is of the form $Cp^2$ for some
constant $C \approx 1.13723$ suggesting that maybe the asymmetric case can be reduced
to the $p=1$ case?
...
\begin{itemize}
    ...
    \item It is possibly true that the optimal strong neighbourhood density bound is
        some function $\lambda(p)=Cp^2$, but this doesn't necessarily imply that the
        best chromatic index bound must be a constant for all $p$.
\end{itemize}
