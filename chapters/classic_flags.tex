...
\begin{example}
    ...
    Then we can see that $c(\trianglemarked; G) + c(\triangletwoedgemarkedcentre)$ counts
    all possible ways of choosing pairs from the neighbourhood of $v$ hence
    $c\left(\trianglemarked; G\right) + c\left(\triangletwoedgemarkedcentre; G\right) = \binom{\deg v}{2}$.
    Therefore:
    \[
    \begin{split}
        p(\edgemarked; G)^2 - p(\edgemarked^2; G)
        &= \left(\frac{c(\edgemarked; G)}{|G|-1}\right)^2
            - p(\trianglemarked; G) - p(\triangletwoedgemarkedcentre; G)\\
        &= \left(\frac{\deg v}{|G|-1}\right)^2
            - \frac{c(\trianglemarked; G) + c(\triangletwoedgemarkedcentre; G)}{\binom{|G|-1}{2}}\\
        &= \left(\frac{\deg v}{|G|-1}\right)^2
...
\begin{example}[Proving Mantel's theorem via SDP]
    ...
    This value of $P$ shouldn't be surprising as $P = \nu\nu^T$ where
