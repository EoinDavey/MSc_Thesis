...
\begin{example}
    ...
    we are counting how many distinct ordered pairs of vertices $(a, b)$ are there in $G$ such that
...
\begin{example}
    The chain rule tells us if $\Gcl$ is all graphs then for any $G$ we have
    \[
...
We've described flags in detail and now understand the link between a partially labelled graph and
the density functions they represent. We would like to then formally
...
\begin{example}
    ...
    We know that $F \geq 0$ for all flags $F$ ($\Gcl^\sigma \subseteq \SemCone^\sigma$).
...
matrices then any such $P$ has spectral decomposition $P=\sum_{i=1}^m \nu_i\nu_i^T$.
Then
\[
